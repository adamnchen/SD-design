package com.sutran.sd.common.utils.redis;
...
public class RedisUtils {
     ...
     * 删除多个对象
     */
    public static boolean deleteMultiObject(final String... keys) {
        for (String key : keys) {
     ...
     * @return 缓存键值对应的数据
     */
    public static <T> boolean delCacheList(final String key,List<T> elements) {
        RList<T> rList = CLIENT.getList(key);
     ...
     * @return 缓存键值对应的数据
     */
    public static <T> boolean delCacheSet(final String key,Set<T> elements) {
        RSet<T> rSet = CLIENT.getSet(key);
        if (CollectionUtil.isNotEmpty(elements)) {
            return rSet.removeAll(elements);
        }
        return true;
    }
     ...
     * @param keys 指定key数组
     */
    public static void deleteKey(final String... keys) {
