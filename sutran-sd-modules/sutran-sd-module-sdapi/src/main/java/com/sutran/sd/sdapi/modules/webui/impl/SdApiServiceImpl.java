package com.sutran.sd.sdapi.modules.webui.impl;
...
import com.sutran.sd.sdapi.domain.dto.task.SdInternalProgressDto;
import com.sutran.sd.sdapi.domain.dto.txt2img.SdApiModelParamDto;
import com.sutran.sd.sdapi.domain.dto.txt2img.SdText2ImgDto;
import com.sutran.sd.sdapi.events.MsgSendThirdEvent;
import com.sutran.sd.sdapi.modules.system.*;
...
import static com.sutran.sd.sdapi.constants.SdApi.*;
...
public class SdApiServiceImpl implements SdApiService {
    ...
    public void refreshLoraModels() {
        ...
        List<LinkedHashMap<String,Object>> loraModelVos = new ArrayList<>();
        try{
            // 刷新模型
    ...
    public void addModelClassify(SdUserModelClassifyDto dto) {
        sdUserModelClassifyService.save(new SdUserModelClassify().setName(dto.getName()).setCrtUserId(LoginHelper.getUserId()).setCrtTime(new Date()));
    }
    @Override
    ...
    public String txt2img(SdText2ImgDto dto) {
        ...
        // 新增任务
        sdUserTaskService.insertTask(taskId,userId,userName,0,0);
        // 请求放入消息队列中
    ...
    public String testTxt2ImgOfLoraModel(SdText2ImgDto dto) {
        ...
        // 新增任务
        sdUserTaskService.insertTask(taskId,userId,userName,2,0);
        // 请求放入消息队列中
        rabbitTemplate.convertAndSend(SD_TXT_TO_IMG_DRAW_EXCHANGE,SD_TXT_TO_IMG_DRAW_ROUTING_KEY,msg,new CorrelationData(taskId));
        return null;
    }
        ...
        try {
            ...
            Forest.post(TXT_TO_IMG_API).address(sdGpuPool.getHost(), sdGpuPool.getPort()).contentTypeJson().addBody(data).connectTimeout(30, TimeUnit.MINUTES)
                .onSuccess((result, req, res) -> {
                    ...
                    // 更新进度状态
                    sdUserTaskService.completeTask(taskId,res.getTimeAsMillisecond());
                    if (isTest) {
                        // 发送完成消息
                        JSONObject wxMsg = new JSONObject();
                ...
                .onError((ex,req,res)->{
                    ...
                    if (ex.getMessage().contains("OutOfMemoryError") || ex.getMessage().contains("cpu and cuda:0") || ex.getMessage().contains("CUDA out of memory")) {
                        ...
                        sdUserTaskService.failTask(taskId,ex.getMessage(),queueTime);
                        JSONObject wxMsg = new JSONObject();
                        wxMsg.put("taskId",taskId);
                        wxMsg.put("type","MODEL_TEST");
            ...
            // 更新进度状态
            sdUserTaskService.failTask(taskId,e.getMessage(), queueTime);
        }
        finally {
    ...
    public String img2img(SdImg2ImgDto dto) {
        ...
        // 新增任务
        sdUserTaskService.insertTask(taskId,userId,userName,1,isRedraw);
