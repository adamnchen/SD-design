package com.sutran.sd.sdapi.mq;
...
public class RabbitConfig {
    ...
    public AmqpTemplate amqpTemplate() {
        ...
        rabbitTemplate.setReturnsCallback(returned -> {
            ...
            log.error("MQ消息：{} 发送失败, 应答码：{} 原因：{} 交换机: {}  路由键: {}", correlationId, returned.getReplyCode(), returned.getReplyText(), returned.getExchange(), returned.getRoutingKey());
        });
        // 消息确认，yml需要配置 publisher-confirms: true
        rabbitTemplate.setConfirmCallback((correlationData, ack, cause) -> {
            if (!ack) {
                log.error("MQ消息发送到exchange失败,原因: {}", cause);
            }
        });
